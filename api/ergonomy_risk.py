...
CORREÇÃO v3: Ajuste no cálculo do ponto 'Neck'. Agora ele é calculado
como um ponto intermediário entre o centróide do rosto e o centróide
dos ombros, atendendo à solicitação de ajuste visual/anatômico.
"""
import cv2
import numpy as np
import mediapipe as mp
...
def draw_angle_on_frame(frame, p1, p2, p3, angle_value, color=(255, 255, 0), radius=30, thickness=2, font_scale=0.6):
    ...
    cv2.putText(frame, f"{angle_value:.2f}°", (text_offset_x, text_offset_y), 
                cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness, cv2.LINE_AA)
    return frame

# --- Função Principal de Processamento de Frame (AJUSTADO PARA PESCOÇO INTERMEDIÁRIO) ---
def process_frame_for_rula(frame, detector_name='mediapipe'):
    ...
    # --- CORREÇÃO PESCOÇO (NECK) ---
    # Recalcula 'Neck' para ser o ponto intermediário entre o rosto e os ombros,
    # em vez de apenas o ponto médio entre os ombros.
    
    # 1. Obter centro dos ombros (Base)
    shoulder_center = None
    if all(key_points.get(k) for k in ['RShoulder', 'LShoulder']):
        r_shoulder = np.array(key_points['RShoulder'])
        l_shoulder = np.array(key_points['LShoulder'])
        shoulder_center = (r_shoulder + l_shoulder) / 2
    
    # 2. Obter centro do rosto (Topo) - Média de Nariz, Olhos e Orelhas disponíveis
    face_points = []
    for part in ['Nose', 'LEye', 'REye', 'LEar', 'REar']:
        if key_points.get(part):
            face_points.append(np.array(key_points[part]))
    
    face_center = None
    if face_points:
        face_center = np.mean(face_points, axis=0)

    # 3. Calcular Neck Intermediário
    if shoulder_center is not None and face_center is not None:
        # Ponto médio entre a "cara" e os "ombros"
        neck_np = (shoulder_center + face_center) / 2
        key_points['Neck'] = (int(neck_np[0]), int(neck_np[1]))
    elif shoulder_center is not None:
        # Fallback se não detectar rosto
        key_points['Neck'] = (int(shoulder_center[0]), int(shoulder_center[1]))
    # Se nem ombro tiver, o código abaixo provavelmente falhará nos cálculos de ângulo de qualquer forma.

    # --- Cálculos de Ângulo e Pontuação ---
    
    # OM BRO / BRAÇO SUPERIOR (direito ou esquerdo)
    if all(key_points.get(k) for k in ['RHip', 'RShoulder', 'RElbow']):
        ...
        frame = draw_angle_on_frame(frame, key_points['RHip'], key_points['RShoulder'], key_points['RElbow'], metrics['upper_arm_angle'], color=(255, 255, 0))
    elif all(key_points.get(k) for k in ['LHip', 'LShoulder', 'LElbow']):
         metrics['upper_arm_angle'] = calculate_angle(key_points['LHip'], key_points['LShoulder'], key_points['LElbow'])
         metrics['upper_arm_score'] = classify_upper_arm(metrics['upper_arm_angle'])
         frame = draw_angle_on_frame(frame, key_points['LHip'], key_points['LShoulder'], key_points['LElbow'], metrics['upper_arm_angle'], color=(255, 255, 0))
    else:
        # print("Ombro: Landmarks insuficientes") # Opcional: silenciar prints excessivos
        pass

    # COTOVELO / BRAÇO INFERIOR
    if all(key_points.get(k) for k in ['RShoulder', 'RElbow', 'RWrist']):
        ...
        frame = draw_angle_on_frame(frame, key_points['RShoulder'], key_points['RElbow'], key_points['RWrist'], metrics['lower_arm_angle'], color=(255, 255, 0))
    elif all(key_points.get(k) for k in ['LShoulder', 'LElbow', 'LWrist']):
         metrics['lower_arm_angle'] = calculate_angle(key_points['LShoulder'], key_points['LElbow'], key_points['LWrist'])
         metrics['lower_arm_score'] = classify_lower_arm(metrics['lower_arm_angle'])
         frame = draw_angle_on_frame(frame, key_points['LShoulder'], key_points['LElbow'], key_points['LWrist'], metrics['lower_arm_angle'], color=(255, 255, 0))

    # TRONCO
    if all(key_points.get(k) for k in ['Neck', 'LHip', 'RHip']):
        # Calcula ponto médio do quadril
        l_hip = np.array(key_points['LHip'])
        r_hip = np.array(key_points['RHip'])
        hip_mid_np = (l_hip + r_hip) // 2
        hip_mid = (int(hip_mid_np[0]), int(hip_mid_np[1])) 
        
        # Referência vertical (Joelho ou projetado)
        ref_point_np = key_points.get('RKnee')
        if ref_point_np is None:
            ref_point_np = tuple(hip_mid_np + (0, 50))
        ...
        frame = draw_angle_on_frame(frame, key_points['Neck'], hip_mid, ref_point, metrics['trunk_angle'], color=(0, 255, 0), radius=50)
    ...
    if all(key_points.get(k) for k in ['Nose', 'Neck', 'LHip', 'RHip']):
        ...
        frame = draw_angle_on_frame(frame, hip_mid, key_points['Neck'], key_points['Nose'], metrics['neck_angle'], color=(255, 0, 0), radius=25)
...
def draw_angle_on_frame(frame, p1, p2, p3, angle_value, color=(255, 255, 0), radius=30, thickness=2, font_scale=0.6):
    """
    #Desenha um arco e o valor do ângulo no frame, corrigido para
    #sempre desenhar o ângulo interno (< 180). p2 é o vértice do ângulo.
    """
    if None in (p1, p2, p3, angle_value):
